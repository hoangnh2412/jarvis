...
export type CraftPdfPageShellProps = {
  title?: string
  description?: string
  showHeader?: boolean
...
export function CraftPdfPageShell({
  title,
  description,
  showHeader = true,
  headerActions,
  ...
  const header =
    showHeader && (title || description || headerActions) ? (
      <Toolbar.Root className="mb-5 flex w-full shrink-0 flex-col gap-3 border-0 border-b border-line bg-transparent p-0 pb-4 sm:flex-row sm:items-center sm:justify-between">
  ...
  return <div className={`flex w-full flex-col ${className}`}>{body}</div>
}
