// Pipeline Step 5: Projects the query to include only requested columns to optimize bandwidth.
...
public static class QueryableColumnExtensions
{
    /// <summary>
        ...
        where TEntity : class, IEntity
    {
        ...
        if (allowedFields == null || allowedFields.Count == 0)
            throw new ArgumentException("Dynamic column selection is blocked because no AllowedFields are configured for this query.", nameof(allowedFields));
        ...
        {
            if (deniedFields != null && deniedFields.Contains(name))
                throw new ArgumentException($"Column '{name}' is blocked by blacklist in projection.", nameof(columns));

            if (!string.Equals(name, "Id", StringComparison.OrdinalIgnoreCase) && !allowedFields.Contains(name))
                throw new ArgumentException($"Column '{name}' is not allowed in projection.", nameof(columns));
