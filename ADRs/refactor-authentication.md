...
| Startup `Key=""` | Môi trường không Development + ApiKey enabled → `ValidateOnStart` fail (cố ý) |
...
### Sample — `SampleApiKeyProvider` / `SampleBasicAuthCredentialProvider` chưa dùng

**Files:** `Sample/Authentication/Sample*Provider.cs`

**Issue:** Stub giữ cho DB/custom lookup sau; Sample hiện wire `Config*Provider`.

**Suggested fix:** Khi có DB, đổi generic trong `AddSampleAuthentication` hoặc chọn provider theo config flag.
...
| `SampleApiKeyProvider` / `SampleBasicAuthCredentialProvider` | Placeholder (Sample) |
