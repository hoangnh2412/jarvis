...
```bash
# từ app
npm i file:../path-to/packages/ui-kit
```

### Publish rồi cài từ registry

```bash
# trong packages/ui-kit
npm login
...
@source "../node_modules/@fe/ui-kit/dist";
@import "@fe/ui-kit/theme.css";
@import "@fe/ui-kit/styles.css";
```
...
// tailwind.config.js
module.exports = {
  presets: [require('@fe/ui-kit/tailwind.preset.cjs')],
  content: [
    './index.html',
    './src/**/*.{js,ts,jsx,tsx}',
    './node_modules/@fe/ui-kit/dist/**/*.{js,cjs}',
  ],
}
```
...
Sau khi sửa kit: `npm run build` trong `packages/ui-kit`, rồi **restart** `npm run dev` ở app.
...
```tsx
import { CrudPage, type TemplateFormData } from '@fe/ui-kit'
