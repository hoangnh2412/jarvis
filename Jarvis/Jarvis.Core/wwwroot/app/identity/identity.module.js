(function () {
  ...
  angular
    ...
    .factory("httpInterceptor", [
      ...
      "sweetAlert",
      "cacheService",
      function ($state, APP_CONFIG, sweetAlert, cacheService) {
        ...
        return {
            ...
            if (response.status === 500 || response.status === 400) {
              if (
                ...
                try {
                  ...
                  if (
                    responseParse.errors &&
                    Object.keys(responseParse.errors).length > 0
                  ) {
                    var err = "";
                  ...
                  if (stringError) sweetAlert.error("Lỗi", stringError);
                }
              ...
              } else sweetAlert.error("Lỗi", response.data.message);
              return response;
            }
          ...
          request: function (config) {
            ...
            var hasIgnore = ignores.filter(function (x) {
              return config.url.endsWith(x);
            });
            if (hasIgnore.length > 0) return config;
    ...
    .factory("permissionService", [
      "cacheService",
      "APP_CONFIG",
      function (cacheService, APP_CONFIG) {
        return {
          hasClaim: function (claim) {
            ...
            if (context)
              for (var prop in context.claims) {
                ...
                if (
                  prop === "Special_TenantAdmin" &&
                  APP_CONFIG.claimOfTenantAdmins.includes(claim)
                ) {
                  return true;
                }
            ...
            if (context)
              for (var prop in context.claims) {
                ...
                for (var i = 0; i < claims.length; i++) {
                  if (
                    prop === "Special_TenantAdmin" &&
                    APP_CONFIG.claimOfTenantAdmins.includes(claims[i])
                  ) {
                    return true;
                  }
          ...
          modules: [
            {
              ...
              serie: true,
              files: [
                "libs/angular/ng-img-crop/ng-img-crop.css",
                "libs/angular/ng-img-crop/ng-img-crop.js",
              ],
            },
            {
        ...
        $transitions.onBefore(
          {
            to: function (state) {
              ...
              var hasPermission = permissionService.hasClaims(
                state.data.authorize
              );
